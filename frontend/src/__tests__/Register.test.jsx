...
//     expect(window.location.href).toBe("https://p2p-final-backend.onrender.com/register2");
//   });
...
jest.mock('../pages/Header', () => () => <div>Mock Header</div>);
jest.mock('../pages/Footer', () => () => <div>Mock Footer</div>);
...
describe('Register Component', () => {
  ...
  test('shows error if email domain is invalid', async () => {
    fireEvent.change(screen.getByLabelText(/email/i), { target: { value: 'test@invalid.com' } });
    fireEvent.change(screen.getByLabelText(/username/i), { target: { value: 'user' } });
    fireEvent.change(screen.getByLabelText(/^password$/i), { target: { value: 'pass123' } });
    fireEvent.change(screen.getByLabelText(/retype password/i), { target: { value: 'pass123' } });
    fireEvent.change(screen.getByRole('combobox'), { target: { value: 'User' } });

    fireEvent.click(screen.getByRole('button', { name: /next/i }));
    await waitFor(() => {
      expect(screen.getByText(/email must end/i)).toBeInTheDocument();
    });
  });
  ...
  test('shows error if passwords do not match', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/username/i), { target: { value: 'user' } });
    fireEvent.change(screen.getByLabelText(/^password$/i), { target: { value: 'pass123' } });
    fireEvent.change(screen.getByLabelText(/retype password/i), { target: { value: 'wrongpass' } });
    fireEvent.change(screen.getByRole('combobox'), { target: { value: 'User' } });
  ...
  test('submits valid form without error', async () => {
    global.fetch = jest.fn(() =>
      Promise.resolve({
        ok: true,
